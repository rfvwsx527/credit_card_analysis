...
# 寫入 MySQL 的資料表名稱;_DB_READY 在 main() 初始化成功後設 True
DB_TABLE = "ptt_articles"
_DB_READY = False
...
def flush_to_csv(records: list[dict], is_first_batch: bool) -> None:
    """將一批資料寫入（或追加）CSV,並同步 append 進 MySQL。

    DB 在 main() 開頭已 ensure 表 + 清空一次,這裡每批只負責 append。
    DB 寫入失敗只記 warning,不影響 CSV。
    """
    ...
    if _DB_READY and not df.empty:
        try:
            from db_common import append_df
            append_df(df, DB_TABLE)
        except Exception as e:
            logger.warning(f"⚠️ 本批寫入 MySQL 失敗 (已保留 CSV):{e}")
...
def main():
    ...
    # 初始化 MySQL:建表(若無) + 清空全部舊資料一次。之後每批 append。
    # 連不上就只跑 CSV,不讓爬蟲中斷。
    try:
        from db_common import ensure_table, truncate_table
        empty_df = pd.DataFrame(columns=CSV_COLUMNS)
        ensure_table(empty_df, DB_TABLE)
        truncate_table(DB_TABLE)
        _DB_READY = True
        logger.info(f"MySQL 已就緒:`{DB_TABLE}` (已清空,準備寫入)")
    except Exception as e:
        _DB_READY = False
        logger.warning(f"⚠️ MySQL 初始化失敗,本次只輸出 CSV:{e}")
